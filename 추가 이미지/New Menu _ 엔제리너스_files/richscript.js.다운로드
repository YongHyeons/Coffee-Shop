/*
 * RICHSCRIPT™ Javascript Framework, 
 * 
 * On jQuery(version 1.5.1+)
 * For Angelinus.co.kr
 * 
...
(function($){
	...
	$.extend(true, String.prototype, {
		...
		escapeXml : function(_removeEnter) {
			var s = this
			.replace(/&/g,"&amp;")
			.replace(/\'/g,"&#039;")
			.replace(/\"/g,"&#34;")
	...
	$.extend(true, Math, {
		...
		toIndex: function(_n, _len) {
			var n = this.toInt(_n), len = this.toInt(_len);
			return (n<0)?len-1:n%len;
		},
		toRanking : function(_n) {
			var s = (""+_n).trim();
			return (s=="1")?s+"st":(s=="2")?s+"nd":(s=="3")?s+"rd":s+"th";
		},
		toOpacity : function(_n) {
			var n = Math.toFloat(_n);
			return Math.toInt(n*10)/10;
		}
	});
	...
	$.extend($.EventEmitter.prototype, {
		...
		once : function(_type, _func) {
			return this.bind(_type, _func, 1);
		},
		on : function(_type, _func, _count) {
			return this.bind(_type, _func, _count);
		},
		off : function(_type) {
			return this.unbind(_type);
		},
	...
	/* Check Global var */
	if (window.EventEmitter==undefined) window.EventEmitter = $.EventEmitter;
